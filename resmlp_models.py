...
class layers_scale_mlp_blocks(nn.Module):
    ...
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU, init_values=1e-4,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=int(4.0 * dim), act_layer=act_layer,
                       drop=drop)
    ...
    def forward(self, x):
        ...
        x += self.drop_path(self.gamma_1 * self.attn(self.norm1(x).transpose(1, 2)).transpose(1, 2))
...
class resmlp_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
                 depth=12, drop_rate=0., Patch_layer=PatchEmbed, act_layer=nn.GELU,
        ...
        self.blocks = nn.ModuleList([
            layers_scale_mlp_blocks(
                dim=embed_dim, drop=drop_rate, drop_path=dpr[i], act_layer=act_layer,
...
def resmlp_12(pretrained=False, dist=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.hub.load_state_dict_from_url(
            url=url_path, map_location="cpu", check_hash=True
        )
